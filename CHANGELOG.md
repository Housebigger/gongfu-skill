...
- **战略库 `semiconductor_outlook/` 专题新增「AI 硬件产业链」系列（知识层）**：`05` 技术架构视角 9 节点全景测绘、`06` 投资产业视角（景气周期位置 / 价值分配 / 国产替代兑现度 / 估值陷阱）、`07` 上中下游关系总图（Mermaid + ASCII）、`08` 景气时序图（景气周期位置 × 催化时间轴的时间维度图）。均经多源检索 + 对抗式核查、**零证券代码、非投资建议**，落点 A 集群（先进制造与硬科技）劳动者岗位/技能方向。引擎/接口/版本号不变。
