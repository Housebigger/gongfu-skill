...
> 当前版本 **v1.5.0** ｜ 升级与变更说明见 [CHANGELOG.md](CHANGELOG.md)
...
- `methodology/` 负责沉淀可复用的方法与主题化认知，`accumulation_settle/` 负责沉淀跨文章经验结晶与伴随式反思，`strategy/` 负责把方法论和规划分析转化为各行各业劳动者可用的行动指南、创业路径、协作方法、成长地图与机会索引。
...
共富参谋（gongfu-skill）是一个 Hermes Agent 插件，把本仓库 1800+ 文件里的行业判断、创业方法、成长规划、协作逻辑和趋势前瞻，蒸馏成一个简约接口——你只需要用大白话描述你的情况，它自动给出看得懂、用得上的判断。
...
ZCode 的 skill 格式与现有的 SKILL.md 完全一致。`agents/zcode-skills/` 是从单一源 `skills/` 生成的（已 gitignore），刚 clone 的仓库先生成一次再复制：

```bash
python scripts/build_packs.py          # 生成 agents/zcode-skills/（运行 bash agents/install.sh 会自动生成）
...
│   └── zcode-skills/      ⚙生成：ZCode skill 文件（7 skill + data）
├── scripts/
│   └── build_packs.py     从单一源 skills/ 生成上述 ⚙ 派生知识包
