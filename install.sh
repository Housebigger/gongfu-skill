...
elif [ -f "$(dirname "$0")/skills/gongfu-advisor/plugin.yaml" ]; then
...
PLUGIN_SRC="$REPO_DIR/skills/gongfu-advisor"
...
# ── 创建符号链接 ──
LINK_TARGET="$PLUGINS_DIR/gongfu-advisor"
...
if command -v hermes &>/dev/null; then
    info "启用插件..."
    hermes plugins enable gongfu-advisor 2>/dev/null && ok "插件已启用" || warn "启用失败，请手动运行: hermes plugins enable gongfu-advisor"
else
    warn "未检测到 hermes 命令，请确认 Hermes Agent 已安装。"
    warn "安装后手动运行: hermes plugins enable gongfu-advisor"
fi
