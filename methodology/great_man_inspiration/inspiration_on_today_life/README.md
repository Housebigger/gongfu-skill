# inspiration_on_today_life

这个主题把毛主席相关文章转译成“普通人在大变局中的现实生活方法论”。

它是当前顺序主线最重要的总入口之一，因为：
- 顺序学习总览放在这里的 `indexes/000-mao-anthology-sequential-reading-overview.md`
- 许多跨主题的主线判断，都会先在 today_life 这里形成第一层落地表达

## 当前实际结构

- `survival_strategy/`：8 篇
- `judgement_and_decision/`：17 篇
- `long_term_growth/`：7 篇
- `cooperation_and_organization/`：39 篇
- `indexes/`
  - `000-mao-anthology-sequential-reading-overview.md`

当前合计：71 篇 article-based 笔记；today_life 主题当前实写覆盖到 001—071，顺序总览窗口也推进到 071。

## 推荐入口

- 总入口：`index.md`
- 顺序总览：`indexes/000-mao-anthology-sequential-reading-overview.md`

## 子目录速览

### `survival_strategy/`
适合看：
- 如何保基本盘
- 如何建根据地
- 如何在压力下留出生存与反攻空间
- 如何在困难阶段主动缩编生活机器，让系统更小、更轻、更能打

### `judgement_and_decision/`
适合看：
- 识势、识人、识友方与识主矛盾
- 如何避免被表面动作和短期缓和带偏
- 如何识破假缓和、软性诱导、谣言与离间

### `long_term_growth/`
适合看：
- 长期成长、阶段推进、理解的反复锻打
- 如何把学习从背结论改成研究现实、研究历史、把理论变成会用的方法
- 如何在成长卡顿时定期整顿自己的学习、判断与表达作风，压住主观主义、小圈子成见与空话习气
- 如何反对复盘、沟通、开会、自我总结和成长表达里的空话、黑话、压人话与无对象表达

### `cooperation_and_organization/`
适合看：
- 如何经营合作
- 如何组织支持面
- 如何在统一、斗争、边界、后果结构之间拿捏分寸
- 如何让表达、帮助和影响真正进入真实多数、真实处境与真实语言

## 当前进度说明

- today_life 主题当前实写覆盖到：`071《关于领导方法的若干问题》`
- 顺序总览窗口当前也推进到：`071《关于领导方法的若干问题》`
- 如果继续按 today_life 顺序向下推进，下一篇应是：`072《质问国民党》`

## 使用提醒

- today_life 不是“杂项目录”，而是顺序主线的现实落地总入口
- 当一篇毛文最直接服务于普通人如何判断、组织、坚持、协作、守边界时，优先落到这里
